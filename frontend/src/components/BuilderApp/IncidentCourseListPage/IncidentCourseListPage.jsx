...
import { fetchIncidentCourseList, deleteIncidentCourse } from '../../../api/builder_api';
import CourseListStats from '../CourseListPage/CourseListStats';
import CourseListFilters from '../CourseListPage/CourseListFilters';
import CourseListItem from '../CourseListPage/CourseListItem';
import CourseListPagination from '../CourseListPage/CourseListPagination';
import CourseListEmpty from '../CourseListPage/CourseListEmpty';
import '../CourseListPage/CourseListPage.css';

const IncidentCourseListPage = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetchIncidentCourseList({
        search: searchQuery,
  ...
  const handleDelete = useCallback(async (slug) => {
    try {
      await deleteIncidentCourse(slug);
      setDeleteSuccess('Курс-инцидент успешно удалён');
    ...
    <main className="course-list">
      <header className="course-list__header">
        ...
        <h1 className="course-list__title">
          <i className="fas fa-graduation-cap course-list__title-icon" aria-hidden /> Все курсы-инциденты
        </h1>
        <p className="course-list__subtitle">Управление курсами-инцидентами на платформе</p>
      </header>
      ...
      <section className="course-list__card">
        <div className="course-list__card-header">
          <h2 className="course-list__card-title">Список курсов-инцидентов</h2>
          <a href={urls.create_course || '/courses/create-course/?is_incident=1'} className="course-list__card-create-btn">
        ...
        <div className="course-list__card-body">
          {items.length > 0 ? (
            ...
            <CourseListEmpty
              createCourseUrl={urls.create_course || '/courses/create-course/?is_incident=1'}
              emptyTitle="Курсы-инциденты не найдены"
              emptyText="На платформе пока нет созданных курсов-инцидентов"
              buttonText="Создать первый курс-инцидент"
            />
          )}
        </div>
      </section>
...
export default IncidentCourseListPage;
