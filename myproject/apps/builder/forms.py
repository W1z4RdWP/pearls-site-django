...
class IncidentForm(forms.ModelForm):
    ...
    initiator_users = forms.ModelMultipleChoiceField(
        queryset=None,
        required=False,
        widget=forms.MultipleHiddenInput(),
        label='Инициатор инцидента'
    )
    
    class Meta:
        model = Incident
        fields = ['title', 'incident_type', 'user', 'assigned_to', 'initiator_users', 'deadline', 'status', 'description']
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Получаем всех пользователей для выбора назначенных и инициаторов
        from django.contrib.auth import get_user_model
        User = get_user_model()
        self.fields['assigned_to'].queryset = User.objects.filter(is_active=True).order_by('last_name', 'first_name')
        self.fields['initiator_users'].queryset = User.objects.filter(is_active=True).order_by('last_name', 'first_name')
