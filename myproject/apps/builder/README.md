...
### Кэширование страниц (инциденты и база знаний)

Для уменьшения нагрузки на БД и ускорения отображения сложных страниц используется кэширование HTML‑ответов поверх стандартного Django‑кэша (`django.core.cache`).

#### Бэкенд кэша

- Бэкенд кэша настраивается глобально в `settings` проекта через `CACHES['default']`.
- В dev/по умолчанию может использоваться `LocMemCache` (in‑memory кэш процесса).
- В продакшене рекомендуется использовать Redis (см. раздел развертывания и настройку `CACHES`).
- Код приложения `builder` работает через абстракцию `django.core.cache` и не зависит от конкретного бэкенда.

#### Страница базы знаний (`LessonMasterDetailView` / `builder/master_detail.html`)

- Класс: `LessonMasterDetailView`
- URL: `/builder/content/` (`name='builder:lesson_master'`)
- Кэшируется **полный HTML‑ответ** (главная страница БЗ) на уровне `get()`:
  - для авторизованных пользователей:
    - `lesson_master_page:user_{user_id}:v{version}:{request.get_full_path()}`
  - для анонимных:
    - `lesson_master_page:user_anon:{request.get_full_path()}`
- Кэш **персонализирован**:
  - разделение по пользователю (`user_id`);
  - разделение по версии кэша пользователя (`v{version}`);
  - разделение по полному URL (учёт параметров `lesson_id`, `new_lesson`, `edited_lesson` и т.п.).
- Используется та же константа TTL, что и для инцидентов:
  - `INCIDENTS_PAGE_CACHE_TIMEOUT` (в проде увеличена до 60 минут).

Важно:
- AJAX‑эндпоинт `?ajax=1`, который возвращает только detail‑блок урока (`builder/includes/_lesson_detail_block.html`), **не кэшируется**:
  - при `ajax=1` всегда собирается свежий контекст и шаблон рендерится без `cache.set/get`;
  - кэш применяется только к полной странице без `ajax=1`.

#### Кэширование страниц инцидентов

Для уменьшения нагрузки на БД и ускорения отображения списков используется кэширование HTML‑страниц инцидентов поверх стандартного Django‑кэша (`django.core.cache`).

#### Список инцидентов (`IncidentListView` / `builder/incidents/incidents.html`)

- Класс: `IncidentListView`
- URL: `/builder/incidents/` (`name='builder:incidents'`)
- Кэшируется **полный HTML‑ответ** на уровне `get()`:
  - ключ кэша для авторизованного пользователя:
    - `incidents_page:user_{user_id}:v{version}:{request.get_full_path()}`
  - ключ для анонимного пользователя:
    - `incidents_page:user_anon:{request.get_full_path()}`
- Кэш **персонализирован**:
  - разделение по пользователю (`user_id`);
  - разделение по версии кэша пользователя (`v{version}`);
  - разделение по полному URL с GET‑параметрами (фильтры, даты и т.п.).
- Время жизни кэша задаётся константой:
  - `INCIDENTS_PAGE_CACHE_TIMEOUT = 300` (5 минут).

Правила:
- Любое изменение GET‑параметров (`date_from`, `date_to`, `status`, `incident_type` и др.) ведёт к новому ключу и новому HTML.
- Для авторизованных пользователей кэш не «перетекает» между разными аккаунтами.

#### Детализация инцидентов (`IncidentDetailListView` / `builder/incidents/incident_detail.html`)

- Класс: `IncidentDetailListView`
- URL: `/builder/incidents/detail/` (`name='builder:incident_detail'`)
- Кэшируется аналогично списку:
  - ключ для авторизованных:
    - `incident_detail_page:user_{user_id}:v{version}:{request.get_full_path()}`
  - ключ для анонимных:
    - `incident_detail_page:user_anon:{request.get_full_path()}`
- Используется та же константа `INCIDENTS_PAGE_CACHE_TIMEOUT`.

Правила:
- Кэш зависит от пользователя, версии и полного набора GET‑параметров (поиск, даты, статусы, фильтры подразделений и т.п.).

#### Версия кэша пользователя и сброс кэша

Чтобы можно было «очищать» кэш страниц без явного перебора всех ключей, используется **версионирование кэша на пользователя**:

- Функция `_get_user_cache_version(user_id)`:
  - читает из кэша ключ вида `user_cache_version:{user_id}`;
  - при отсутствии возвращает `1`;
  - эта версия встраивается в ключи страниц (`...:v{version}:...`).

- Вьюха `clear_user_cache`:
  - доступна только авторизованным (`@login_required`);
  - увеличивает версию по ключу `user_cache_version:{user_id}` на 1;
  - показывает пользователю сообщение: «Кэш страниц для вашего профиля был очищен.»;
  - редиректит обратно на `HTTP_REFERER` или на `home`, если заголовка нет.

- URL для сброса кэша:
  - путь: `/builder/incidents/clear-user-cache/`
  - имя: `builder:clear_user_cache`.

- Кнопка в футере (`templates/layout.html`):
  - отображается только для авторизованных пользователей;
  - представляет собой `POST`‑форму на `builder:clear_user_cache` с CSRF‑токеном;
  - по клику очищает кэш страниц для конкретного пользователя (через увеличение версии).

Особенности:
- Старые записи кэша с предыдущими версиями ключей остаются в хранилище, но больше **не используются** кодом (natural expiration).
- Реальное удаление старых ключей возлагается на выбранный бэкенд (Redis, LocMem и т.п.) и его политику очистки.

#### Рекомендации по добавлению нового кэширования страниц

При добавлении кэширования для новых страниц в `builder` рекомендуется:

1. **Всегда учитывать пользователя**, если контент зависит от роли/прав/профиля:
   - включать `user_id` в ключ;
   - использовать версию `v{_get_user_cache_version(user_id)}`.
2. **Включать в ключ полный URL** (`request.get_full_path()`), если страница зависит от GET‑параметров (фильтров, поиска).
3. **Использовать общую константу TTL** (или аналогичную) и не «зашивать» таймауты в строку.
4. **Повторно использовать механизм `clear_user_cache`**:
   - новые страницы, кэшируемые по схеме `...:user_{user_id}:v{version}:...`, автоматически будут очищаться той же кнопкой.
5. При необходимости **разделять «анонимные» и «персональные» страницы**:
   - для анонимных достаточно префикса `user_anon` и URL без версии.
...
- Создание/редактирование уроков (staff/superuser), запрет для mentor; версионирование
- Поиск по категориям/урокам (права доступа)
- Создание/редактирование/удаление курсов и траекторий, операции с курсами в траектории
